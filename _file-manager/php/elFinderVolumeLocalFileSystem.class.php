...
class elFinderVolumeLocalFileSystem extends elFinderVolumeDriver {
	...
	protected function _unpack($path, $arc) {
		...
		{
			$zip = new ZipArchive;
			if ($zip->open($this->_basename($path)) === TRUE) { /* $this->_basename($path) - failo vardas */
	...
	protected function _archive($dir, $files, $name, $arc) {
		...
		function ZipIt($source, $destination, $files)
		{
			...
			if (is_dir($source) === true)
			{
				...
				foreach ($files as $file)
				{
					...
					if (is_dir($file) === true)
					{
						...
						/* FIX 2012.12.12 */
						$iterator = new RecursiveIteratorIterator(new RecursiveDirectoryIterator(str_replace($source . '/', '', $file . '/')));
