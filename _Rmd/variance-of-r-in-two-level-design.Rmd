...
\end{aligned}
$$
The between sums-of-squares are a little bit trickier. From Searle, Casella, and McCulloch (1992), 
$$
\begin{aligned}
\text{E}\left(SS^B_{xx}\right) &= f_1 \phi_x^2 + (m - 1)\sigma_x^2 \\
\text{E}\left(SS^B_{yy}\right) &= f_1 \phi_y^2 + (m - 1)\sigma_y^2 \\
\text{E}\left(SS^B_{xy}\right) &= f_1 \phi_{xy} + (m - 1)\sigma_{xy} \\
\text{Var}\left(SS^B_{xx}\right) &= 2\left[(m - 1)\sigma_x^4 + 2 f_1 \sigma_x^2 \phi_x^2 + f_2 \phi_x^4\right] \\ 
\text{Var}\left(SS^B_{yy}\right) &= 2\left[(m - 1)\sigma_y^4 + 2 f_1 \sigma_y^2 \phi_y^2 + f_2 \phi_y^4\right] \\
\text{Var}\left(SS^B_{xy}\right) &=  \\
\text{Cov}\left(SS^B_{xx}, SS^B_{yy}\right) &=  \\
\text{Cov}\left(SS^B_{xx}, SS^B_{xy}\right) &=  \\
\text{Cov}\left(SS^B_{yy}, SS^B_{xy}\right) &= ,
\end{aligned}
$$
where
$$
