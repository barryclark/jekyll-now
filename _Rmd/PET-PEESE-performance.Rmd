...
Both tests maintain the correct Type-I error rate. (Any variation in rejection rates is just Monte Carlo error.) Thus, it seems pretty clear that if we want to test for small-sample bias, SPET or SPEESE should be used rather than PET or PEESE.
...
bias_plot <- function(dat) {
  ggplot(dat, aes(mean_effect, est_M, color = estimator, shape = estimator)) + 
    ...
    theme_light() + 
    theme(legend.position = "bottom") + 
    labs(color = "", shape = "", x = "Mean effect size", y = "Expected value of estimator")
}
...
# plot_dat <-
#   estimator_performance %>%
#   filter(estimator %in% c("FE-meta","PEESE","PET-PEESE","SPET","SPEESE","SPET-SPEESE")) %>%
#   group_by(study_dist, maximum_n) %>%
#   summarize(panel = panel(bias_plot(dat = .)))
# 
# plot_dat %>%
