...
The question is then how to approximate the distribution of $D$ under $H_0$. El Barmi and Johnson (2006) present a solution to a much more general hypothesis testing problem where the null can be expressed in terms of inequalities in linear combinations of the parameters in a set of independent multinomial distributions. The present case is particularly simple because a) it involves binomial rather than multinomial distributions and b) each constraint pertains to a different, independent distribution. With these simplifications, El Barmi and Johnson's results imply that $D$ follows a weighted sum of $\chi^2$ distributions with degrees of freedom ranging from 0 to $M$, where the weights correspond to the values of the binomial probability mass function with size $M$ and probability 1/2. More precisely,

$$
\text{Pr}(D > d) \approx \frac{1}{2^M}\sum_{m = 0}^M {{M}\choose{m}} \text{Pr}\left(\chi^2_m > d\right).
$$

This expression can be used to directly compute a p-value for an observed value $d$ of the LRT statistic. It is a large-sample approximation, valid as $N \to \infty$, and so it is of course prudent to check how well it holds for the sort of sample sizes we're dealing with. 
...
(p <- rbinom(5, size = 2000, prob = 0.05) / 2000)
...
p_LRT <- function(p, N, alpha, M = length(p)) {
  ...
  a_vals <- dbinom(0:M, size = M, prob = 1/2L)
  chisq <- pchisq(D, df = 0:M, lower.tail = FALSE)
  p_val <- sum(a_vals * chisq)
  c(D = D, p = p_val)
}

p_LRT(p = p, N = 2000, alpha = 0.05)
...
```{r}
res <- r_DP(alpha = 0.05, N = 2000, M = 20, scale_down = 0, reps = 1000)
hist(res$p)
```
...
3. Calculate a p-value by taking the proportion of simulated LRT statistics that are larger than the observed, i.e., $\displaystyle{p = \frac{1}{R} \sum_{r=1}^R I(d^*_r > d)}$. 
...
p_LRT_sim(p = p, N = 2000, alpha = 0.05, plot = TRUE)
```
...
res <- r_DP_sim(alpha = 0.05, N = 2000, M = 5, scale_down = 0, reps = 1000)
hist(res$p)
