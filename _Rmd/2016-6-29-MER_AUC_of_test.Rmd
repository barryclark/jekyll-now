...
\right.
\]
and that decision making with $D$  is as follows:
\[\text{output} = \left\{
  \begin{array}{lr}
    1 & : D > \kappa\\
    0 & : D \leq \kappa\\
...
Lets look at other extreme cases of complete overlap and complete seperation between the positive and negative group. For complete overlap we set $a=0$ and for complete separation we might either reduce $b^2$ or set $a=4$.
...
The AUC is also defined as follows (we will assume that positive results are less than negative results)
\[
AUC=\Pr(\{D|Y=1\}>\{D|Y=0\})=\Phi\left(\frac{2a}{\sqrt{2}b}\right)
\]
Where $\Phi$ is the cummulative standard normal function.

For the simulated data above, let $\kappa=0$ (note this is an arbitrary choice of $\kappa$). 

We start with case $a=2$ and $b=1$
```{r,warning=FALSE}
a=2
kappa=0
#theoritical/truth
...
D_pos=rnorm(N,a,b)
D_neg=rnorm(N,-a,b)
MER_emp[i]=min(sum(D_pos<kappa)/N,0.5)+min(sum(D_neg>kappa)/N,0.5)
#Note that MER_emp can't be bigger than 1 or less than 0
...
```
We will now verify two situations

What is the relationship between the AUC and MER as functions of $a$

What is the relationship between the AUC and MER as functions of $b$


Note that this questions can not be answered directly since the MER also depends on $\kappa$ while the AUC does not. For the moment we leave $\kappa=0$ so that both functions are independent of $\kappa$. We will come to this choice latter. 

Fix $b=1$ and rememeber that we are still assuming that $D|Y=0\leq D|Y=$ on average without loss of generality. 
