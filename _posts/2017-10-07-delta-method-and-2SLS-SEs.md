...
The HC0 variance estimator of the 2SLS estimator is then 
...
where $$\tilde{e}_i = \ddot{y}_i - \ddot{t}_i \hat\delta_1$$. Note that these residuals are calculated based on $$\ddot{t}_i$$, the _full_ treatment variable, not the fitted values. The full treatment variable can be expressed as $$\ddot{t}_i = \tilde{t}_i + \ddot{u}_i$$, by which it follows that 
