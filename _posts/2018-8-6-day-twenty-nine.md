...
I also looked back at the [WFC3 Module Manual](http://americano.dolphinsim.com/dolphot/dolphotWFC3.pdf) and read its section on dolphot's output. The only difference between our output versus a normal dolphot output is the fact that we will have combined photometry for each filter (F606W and F814W) before photometry results for each image. There are also calibrated
instrumental magnitudes and transformed magnitudes provided, along with a calibrated count rate and uncertainty. 

# Conclusion
I'm leaving for today, so I'm posting this now, but I'll let dolphot finish tonight and probably run photometry on one more folder. I'll write tomorrow whether or not I can understand what the output says. 
