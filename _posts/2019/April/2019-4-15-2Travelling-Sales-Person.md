...
I would love to do some more analysis, but more importantly I think playing with the Travelling Salesperson Problem is more important, [tsp-single-file.py]({{ site.url }}/assets/tsp/tsp-single-file.py) with the code, and the progression over the course of a 12 node graph and a `cutofflimit` 400,000 which is computed in less than a second on my computer.
