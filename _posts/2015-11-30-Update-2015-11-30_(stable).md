...
Archlinux decided to go with Xorg-Server 1.18.0 even with the fact that Catalyst doesn't support that server yet. Therefor we used once more our overlay features and blocks that series from updating. However I already updated some drivers against the 1.17 series of the Xorg-Server, including the latest Nvidia drivers.
...
More changes in regard of Deepin and Cinnamon got introduced with this update. We are now using more upstream Archlinux packages for both desktop environments. Python and Haskell got again some updates and Thunderbird is now at 38.4.0. The second release candidate of Wine 1.8 completes this update.
