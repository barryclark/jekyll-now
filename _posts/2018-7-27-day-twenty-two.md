...
# DOLPHOT
So, while writing the new parameter file last night, I had a very important epiphany about how to run DOLPHOT. I was in error before because I had no refernce image and tried to do every file that I have at once. No one said it explicitly anywhere, but I've figured out from reading the [manual](https://github.com/GosnellResearchGroupSummer2018/NGC6819/blob/master/dolphot_userguide.pdf) and that [PYDOLPHOT](https://github.com/dweisz/pydolphot) tutorial 1000 times that you need a drizzled image as the reference image and then you can do photometry on all the flc images that the drizzled image is a composite of. I never got DOLPHOT to work on my laptop, so I tried to run it with the new parameter file on the Macs in the computational lab with that epiphany in my head. The changes I made to the parameter file follow (everything is set to the default DOLPHOT settings except for these things):
...
So, the epiphany I mentioned at the beginning means that I have to completely change up my strategy for running DOLPHOT. It also made me realize that those were actually error messages I got the first time I ran DOLPHOT. The real output should tell me which .fits file it's analyzing at any given time and then tell me how many stars it's found as it's finding them. The next step is to go back to MAST and get drizzled images again then all their flc components and put the associated files in a unique folder for each drizzled image. I also forgot that when I installed DOLPHOT on this computer (the Mac), I only got the F606W PSFs because I was confused at the time, so I went back and got the F814W images, too (with both flc and drz, there's 74 images).
...
It ALMOST works. So close. For a second, I was having the 15 chip issue, but realized I should have been using `wfc3mask`, not `acsmask`, and that fixed it. It's working great for flc images, but I need to modify a parameter file (a different one than above) and maybe add some other things to make it work with drizzled images. I'm not exactly what all I have to do yet. 
...
Rory and I were also wondering if `wfc3mask` works with batches, and I think the answer is yes, but it wasn't yet working for me because I have flc and drz images in the same batch and I can't get `wfc3mask` to work with the drizzled pictures yet. Moreover, I'm almost certain you can't use the same `wfc3mask` parameters put a mask on flc and drz images so I'll have to do them separate. 

Finally, I mentioned that I never got DOLPHOT working on my laptop. The issue is that not all of the DOLPHOT commands compiled when I ran the `make` command (though all the WFC3 and ACS module commands compiled). The ~10% of commands that did compile work just fine, which is why I thought all was well, until the bash couldn't find splitgroups. I talked to Marta who knows more about programming with a PC than I do, and she thought that cygwin likely didn't install all the developer tools that DOLPHOT needs to compile properly, which makes a lot of sense to me. I ran into that issue earlier (since at first the `make` command wasn't installed with cygwin), but hadn't made the connection to the current situation. Once I get home, I'll try getting the rest of the things I need from cygwin. Marta gave me a list of things she needed to compile astrometry.net and found another of essential developer tools for cygwin, which is really helpful. 
