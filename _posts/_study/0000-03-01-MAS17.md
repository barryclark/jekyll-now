...
$\sin \theta _r=\frac{n_1\sin \theta _i}{n_2}$sinθr​=
n1​sinθi​
n2​​​
...
(1) 프레넬 방정식(Fresnel equations) 또는 프레넬 공식(Fresnel`s formulas)은 반사계수와 투과계수에 관한것으로 한 매질과 광학적 특성 즉, 굴절률이 다른 매질의 계면에서 반사 또는 투과 진폭을 입사진폭으로 나눈 값을 말한다. 프랑스의 물리학자 오귀스탱 장 프레넬이 유도하였다. 

 https://ko.wikipedia.org/wiki/%ED%94%84%EB%A0%88%EB%84%AC_%EB%B0%A9%EC%A0%95%EC%8B%9D
...
물질의 흡수력과 구성이 빛의 굴절이 일어나는 일을 결정합니다. 금속은 매우 높은 흡수력을 가지고 있습니다. 따라서 물질에서 나오는 굴절이 없습니다. 다른 의미로, 물질은 유리는 매우 낮은 흡수율을 가지고 있습니다. 따라서 굴절된 빛은 볼륨을 온전히 통과할 수 있습니다. 흡수 없이 말입니다.
 ...
 쉐이딩에서 디퓨전을 모델링하는 것은 표면 아래에서 산란하는 크기에 의해 결정됩니다. 만일 모든 거리가 (모든 광선이 표면에서 들어오는 진입과 나가는 부분까지의 거리) 픽셀보다 작다면, 지역적으로 diffusion을 계산할 수 있습니다. 아니라면, 특별한 non-local subsurface scattering 랜더링 기술이 필요합니다. [1]에 나와있습니다. 예를 들어 사람의 피부와같은, 왁스 또는 충분히 얇은 유전체 물질등이요.
...
$L_0\left(w_0\right)=\int _{\Omega }^{\ }f\left(w_0,\ w_i\right)\otimes L_i\left(w_i\right)\left(\underline {n\cdot w_i}\right)dw_i$L0​(w0​)=∫
 
Ω​f(w0​, wi​)⊗Li​(wi​)(n·wi​
)dwi​​
...
어떻게 찾았는가? Incoming light direction을 쳐서 찾았다.
...
$f\left(w_0,\ w_i\right)=f\left(w_i,w_0\right)$f(w0​, wi​)=f(wi​,w0​)​
 그리고 에너지가 보존되어야 합니다.

$\forall w_i,\ \int _{\Omega }^{\ }f\left(w_0,w_i\right)\left(\underline {n\cdot w_i}\right)dw_0\le 1$∀wi​, ∫
...
$f_{Cook-Torrance}\left(v,\ l\right)=\frac{F\left(l,h\right)G\left(l,v,h\right)D\left(h\right)}{4\left(n\cdot l\right)\left(n\cdot v\right)}$fCook−Torrance​(v, l)=
F(l,h)G(l,v,h)D(h)
4(n·l)(n·v)​​
...
$F_{Schlick}\left(F_0,\theta \right)=F_0+\left(1-F_0\right)\left(1-\cos \left(\theta \right)^5\right)$FSchlick​(F0​,θ)=F0​+(1−F0​)(1−cos(θ)5)​
 여기서 specular BRDF 및 microfacets의 경우입사각은 다음과 같습니다.

$\cos \left(\theta \right)=l\cdot h$cos(θ)=l·h​
$F_{Schlick}\left(F_0,l,h\right)=F_0+\left(1-F_0\right)\left(1-\left(l\cdot h\right)^5\right)$FSchlick​(F0​,l,h)=F0​+(1−F0​)(1−(l·h)5)​
 어떤 물질의 기본 반사는 들어오는 빛의 파장(wave length)의 함수가 일 수 있습니다(be). 이 현상은 금이나 구리의 색을 부여합니다(gives). PBR에서 F의 RGB 삼중항(triplet (term))으로 모델링 됩니다.(is modeled)
...
3.3.3 Normal distribution term
...
$isotropic\ case\ NDF`s\ normalization\ factor\ =\ \frac{1}{\alpha ^2\pi }$isotropic case NDF`s normalization factor = 
1
α2π​​
$D_{GGX}\left(h,\alpha \right)=\frac{\alpha }{\pi \left(\left(n\cdot h\right)^2\left(a^2-1\right)+1\right)^2}$DGGX​(h,α)=
α
π((n·h)2(a2−1)+1)2​​
 다른 중요한 모델은 Beckmann` NDF입니다.

$D_{Beckmann}\left(h,\alpha \right)=\frac{1}{\pi \alpha ^2\left(n\cdot h\right)^4}\exp \left(\frac{\left(n\cdot h\right)^2-1}{\alpha ^2\left(h\cdot h\right)^2}\right)$DBeckmann​(h,α)=
1
πα2(n·h)4​exp(
(n·h)2−1
α2(h·h)2​)​
...
$G_{Smith}\left(l,v,h\right)=G_1\left(l\right)G_1\left(v\right)$GSmith​(l,v,h)=G1​(l)G1​(v)​
 G_1은 GGX같은 다른 모델이 될 수 있습니다.

$G_{GGX}\left(e\right)=\frac{2\left(n\cdot e\right)}{\left(n\cdot e\right)+\sqrt{a^2+\left(1+a^2\right)\left(n\cdot e\right)^2}}$GGGX​(e)=
2(n·e)
(n·e)+√a2+(1+a2)(n·e)2​​
또는 Beckmann에서는.

$c=\frac{n\cdot e}{\alpha \sqrt{1-\left(n\cdot e\right)^2}}$c=
n·e
α√1−(n·e)2​​
$G_{Beckmann}\left(e\right)=\begin{cases}3.535c\ +2.181c^2&if\ c<1.6\ \\1&if\ c\ \ge 1.6\end{cases}$GBeckmann​(e)={
3.535c +2.181c2	if c<1.6 
1	if c ≥1.6
​
...
3.4 Lambert`s diffuse BRDF

 실시간 렌더링에서, 가장 자주 사용되는 diffuse BRDF는 Lambert`s BRDF입니다.

$f_{Lambert}\left(v,l\right)=\frac{c_{diff}}{\pi }$fLambert​(v,l)=
cdiff​
π​​
...
$\cos \left(\theta \right)=v\cdot n$cos(θ)=v·n​
$F_{diff}\left(F_0,v,n\right)=F_0+\left(1-F_0\right)\left(1-\left(v\cdot n\right)^5\right)$Fdiff​(F0​,v,n)=F0​+(1−F0​)(1−(v·n)5)​
마지막으로, 전체 BRDF는

$f\left(v,l\right)=f_{Lambert}\left(v\cdot l\right)\left(1-F_{diff}\right)+f_{Cook-Torrance}\left(v,l\right)F_{diff}$f(v,l)=fLambert​(v·l)(1−Fdiff​)+fCook−Torrance​(v,l)Fdiff​​
...
$T\left(c_{hdr}\right)=1-\exp \left(-c_{hdr}\cdot \exp osure\right)$T(chdr​)=1−exp(−chdr​·exposure)​
...
4.2 Solving the reflectance equation

 이 섹션은 가능한 해결법과 랜더링 방정식의 근사치를 제공합니다.

​

4.2.1 Punctual light sources
...
$\left(4.2\right)\ \ \ \ L_0\left(v\right)=f\left(v,l_c\right)\otimes L_i\left(l_c\right)\left(\underline {n\cdot l_c}\right)$(4.2)    L0​(v)=f(v,lc​)⊗Li​(lc​)(n·lc​
)​
...
$\left(4.3\right)\ \ \ \ l_c=n$(4.3)    lc​=n​
$\left(4.4\right)\ \ \ \ c_{light}=\int _{\Omega }^{\ }f_{Lambert}\left(v,w_i\right)\otimes L_i\left(w_i\right)\left(\underline {n\cdot w_i}\right)dw_i$(4.4)    clight​=∫
 
Ω​fLambert​(v,wi​)⊗Li​(wi​)(n·wi​
)dwi​​
(3.2)를 (4.4)에 c_diff=1과 함께 대입하면, ((3.2)가 뭐드라???)

$\left(4.5\right)\ \ \ \ c_{light}=\frac{1}{\pi }\int _{\Omega }^{\ }L_i\left(w_i\right)\left(\underline {n\cdot w_i}\right)dw_i$(4.5)    clight​=
1
π​∫
...
$c_{light}=\ \frac{1}{\pi }L_i\left(l_c\right)$clight​= 
1
π​Li​(lc​)​
$L_i\left(l_c\right)=\pi c_{light}$Li​(lc​)=πclight​​
(4.7)을 (4.2)에 집어넣으면 punctual light shading equation의 최종 결과를 얻습니다.

$L_o\left(v\right)=\pi f\left(v,l_c\right)\otimes c_{light}\left(\underline {n\cdot l_c}\right)$Lo​(v)=πf(v,lc​)⊗clight​(n·lc​
)​
...
$c_{light}\left(d\right)=\frac{c_{light_0}}{a_0d^2+a_1d+a_2}$clight​(d)=
clight0​​
a0​d2+a1​d+a2​​​
​
...
$\left(4.11\right)\ \ \ \ I\left(n\right)=\frac{1}{\pi }\int _{\varphi }^{\ }\int _{\theta }^{\ }L_i\left(\varphi _i,\theta _{i\ }\right)\cos \left(\theta _i\right)\sin \left(\theta _i\right)d\theta _id\varphi _i$(4.11)    I(n)=
1
π​∫
 
φ​∫
 
θ​Li​(φi​,θi ​)cos(θi​)sin(θi​)dθi​dφi​​
 적분은 Monte Carlo 적분을 각각적용하여 풀 수 있습니다.

$\left(4.12\right)\ \ \ \ I\left(n\right)=\frac{1}{\pi }\ \frac{2\pi }{N_1}\ \frac{\pi }{2N_2}\sum _{\ }^{N_1}\sum _{\ }^{N_2}L_i\left(\varphi _i,\theta _i\right)\cos \left(\theta _i\right)\sin \left(\theta _i\right)$(4.12)    I(n)=
1
π​ 
2π
N1​​ 
...
N2​
∑
 Li​(φi​,θi​)cos(θi​)sin(θi​)​
$\left(4.13\right)\ \ \ \ I\left(n\right)=\frac{\pi }{N_1N_2}\sum _{\ }^{N_1}\sum _{\ }^{N_2}L_{i\ }\left(\varphi _i,\theta _i\right)\cos \left(\theta _i\right)\sin \left(\theta _i\right)$(4.13)    I(n)=
π
N1​N2​​
N1​
∑
...
$t=\left(\sin \left(\theta \right)\cos \left(\varphi \right),\sin \left(\theta \right)\cos \left(\varphi \right),\cos \left(\theta \right)\right)$t=(sin(θ)cos(φ),sin(θ)cos(φ),cos(θ))​
...
뒷부분에 체력떨어져서 구글번역기좀 돌렸지만,, ㅎㅎ. 그래도 결론까지 함.

수정하고 평가하고 다시하고 수정하고 평가하면서 좀더 잘 할 수 있겠지이이이.