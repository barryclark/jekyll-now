...
After struggling to relearn Javascript to perform http requests and return results in the browser (I really wish everything was like Python's requests...) I had a minimum viable product for my friends to test out.

Everything was working, there was a form that would perform an AJAX request to my server, get the search results, and post them on the browser.
...
Using Python's requests library (woohoo!) I performed a GET request to the Youtube search endpoint, collected the html response, then parsed it with Beautiful Soup to pull the resulting video information from the results page.
