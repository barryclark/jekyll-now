...
description: I started working on MQTT unexpectedly while helping a friend on a facial recogination camera with temperature detecting capabalities. While he was using HTTP GET/POST methods to send data to the hub. which is great!, its working and the result are getting on time. But somthing bugged. what if we can make this even better and faster. well here we are looking into MQTT. Join with me for an adventure ..... >.<
