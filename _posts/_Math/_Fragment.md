...
두 벡터 v1, v2사이의 회전각은 벡터 v1이 v2로 변했다고 했을 때 몇 도나 회전 했고 또 어떤 방향으로 회전했는지 계산합니다.

두 직선 L1과 L2가 이루는 각은 두 점 (a1, b1), (a2, b2)를 지나는 직선 L1과 en wja (c1, d1), (c2, d2)를 지나는 직선 L2가 이루는 사이각을 말합니다.

두 벡터가 이루는 절대적인 사이각은 비교적 쉽게 구할 수 있지만 v1이라는 벡터가 v2로 변했을 때, 몇 도나 움직였고 또 어떤 방향으로 변했는지까지 알아내려면 머리가 조금 복잡해 집니다.

## 벡터의 내적
두 벡터 v1 = (x1, y1), v2 = (x2, y2)에 대해 그 사이각을 θ라 했을 때, 두 벡터의 내적(inner product 또는 dot product)는 다음과 같이 정의됩니다.

대수적 정의 : v1·v2 = x1x2 + y1y2 
기하학적 정의 : v1·v2 = |v1||v2|cosθ

## 벡터의 외적
벡터의 외적은 기본적으로 3차원에서 정의되며 두 벡터를 v1 = (x1, y1, z1), v2 = (x2, y2, z2)라 했을 때, 두 벡터의 외적(cross product)는 다음과 같이 정의됩니다.

대수적 정의 : v1×v2 = | i j k || x1 y1 z1 || x2 y2 z2 | 
= y1z2 - z1y2, z1x2 - x1z2, x1y2 - y1x2)

단, i=(1,0,0), j=(0,1,0), k=(0,0,1)는 좌표축 단위벡터.

기하학적으로 봤을 때, 외적 v1×v2는 v1, v2로 이루어지는 평면에 수직이면서 그 크기가 (v1, v2)로 이루어지는 평행사변형의 넓이인 |v1||v2|sinθ를 나타냅니다.
...
## 두 벡터의 사이각(회전각)
3차원 공간에서는 어떤 방향에서 봤느냐(이쪽에서 봤는지 반대편에서 봤는지)에 따라서 회전의 방향이 서로 반대가 될 수 있기 때문에 각의 방향을 부호로서 정할 수가 없습니다. 따라서 3차원에서는 두 벡터 v1 = (x1,y1,z1), v2 = (x2,y2,z2)의 절대적인 사이각은 내적을 이용하여 구하고 각의 방향은 외적 벡터의 방향을 보고 따로 판단해야 합니다.. 두 벡터의 (부호가 없는) 절대적인 사이각은 다음과 같이 계산됨:
