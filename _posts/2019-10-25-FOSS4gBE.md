...
## OGC Standards
I was excited to hear from Bart de Lathouwer (OGC President) that OGC is working on RESTful OpenAPI standards for features, coverages and maptiles. Existing standards such as WMS are very widely used, but old (WMS is from 1999!). As we at Nelen & Schuurmans are continuously working on RESTful APIs of geospatial data, I will start following this more closely.


## SAGA
Johan van de Wauw showed how to use [SAGA](http://www.saga-gis.org) and how many raster and vector processing utilities there are. There even is a Python wrapper, I will check that out and see if there are any utilities that we can expose into Python in the way we did it in PyGEOS.
