...
The group of developers first learned about Amazon SageMaker and then went on to build a chatbot in Amazon Lex. The data scientists decided that, instead of following the Lex demo, they'd rather get stuck into the time series forecasting and product recommendation demos. These were more relevant to the machine learning domains in which they were working, and required a bit more data processing.
