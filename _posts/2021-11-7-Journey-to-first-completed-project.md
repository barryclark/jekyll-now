...
```
As you can see, it's basically programming how the application should be built at the compilation/linking level, givin the all the posibilities needed to create the before mentioned operating system. I know, linking all the source files is missing 

## The journey of building the application

Once I had a basic idea of what functions where needed to build an application I just started writting code. Slowly but surely I started executing lua files with *liblua*, started giving access to some C++ functions to lua and had a basic prototype running. I worked before with linux and unix system calls before, so creating and managing processes was not difficult for me, so after having a lua virtual machine running the prototype was basically done.

Of course, without any planning or previous knowledge of working with *lua*, as expected the code was a mess. Not only that, but I did not play with proper modern C++ before, so smart pointers where totally missing, with all the consequences. So the next phase whas to back down on all the written code, and fix all the memory leaks and use after frees I had in code. Finally! After some time of removing all the `new`s and all the `malloc`s and only using smart pointers, I had a program without any memory leaks!
...
Using github's pull request and review system helped me to better organize features, work on multiple features at a time and, I know this sounds weird, review my own code. This point was when I realized that I often write stupid things, and giving me another stage to take a look at the code, and catch this stupid things.
...
## Conclusions
As said before, I find a bit tedious to use my own build system, and I don't really know how to improve that situation... so the project won't be followed until I find time and a better design. 
