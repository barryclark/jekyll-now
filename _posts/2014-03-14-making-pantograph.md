...
$$\vec{A} = \Big(0,\ 0\Big) \qquad \vec{B} = \Big(0,\ (r_a + d + r_b)\Big)$$
$$\vec{P} = \vec{A} + \Big( r_a\ \angle\ \theta_a \Big) \qquad \vec{Q} = \vec{B} + \Big( r_b\ \angle\ \theta_b \Big)$$
...
$$\varphi_a = \tan^{-1}\left( \dfrac{R_y - P_y}{R_x - P_x} \right) \qquad 
\varphi_b = \tan^{-1}\left( \dfrac{R_y - Q_y}{R_x - Q_x} \right)$$

We can then write nodes $S$ and $T$ as
$$\vec{S} = \vec{R} + \left( \ell_2\ \angle\ \varphi_b\right) \qquad
...
$$\vec{N} = \frac{1}{2}\left(\vec{S} + \vec{T}\right) \qquad
d = \sqrt{(\ell_3)^2 - c^2} \qquad
