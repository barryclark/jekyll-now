...
One of the known regularization methods is ensemble learning, in such a method you get to train different large networks and average their predicitons, obviosly this technique requires [high computation resourecs as you are training many networks] [ and introduce slow training process]. Dropout addresses these two problems, dropout is similar to ensemble learning in terms of having many networks trained, but it's totally different in the process.

In dropout, random units are dropped in each training step (each iteration), result with a different smaller network (thinned network), the forward and backward [ propagatoins ] are [ done ] only on this specific thinned network during this specific iteration, as a result, when accumelating the training steps of all thinned networks it's as if we are getting the average of predictions of different networks but with less computations and time.
think of it this way, each thinned network update the weights [sligtly ] and pass them to the next thinned network [etc ], this means a thinned network starts where the previous thinned network stopped!.
and weights continues updating in this way
## Deeper intuition
