...
Linux (필자는 Naver Cloud Platform & AWS server를 이용중이다.) 에<br>
Oracle JDK Binary Source를 다운로드하는 일이 잦다.

아마 wget이나 curl을 통해서 tar.gz 파일 (또는 rpm 파일)을<br>
직접 받는걸 시도해본 분들은 다들 봤을 문구가 필자를 너무 화나게 했다.
...
이런 header를 추가해줘도 쉽사리 Download 되지는 않았다.<br>
(필자가 실력이 없어서 그럴수도 있지만...)


저 Authorized failed 라는 문구를 피하기 위해서 머리를 굴리기 시작했다.<br>
그러다 문득 떠오른 한 곳.
