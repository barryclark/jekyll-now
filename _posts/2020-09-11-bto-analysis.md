...
Some context for any non-Singaporean readers - a Build-to-Order BTO flat is a public housing option that many Singaporean couples purchase as their first home. Typically, you would book a unit which will be completed only a few years later. BTO flats are (usually) released every quarterly. There's just one twist: Those who want to purchase a BTO have to enter a ballot for a unit, and given that demand usually exceeds supply, there's some serious luck* element involved here...

_(* Not entirely accurate as there are some priority schemes available. But for the average applicant-couple luck does matter a lot.)_

I haven't had much luck in getting a good BTO queue number. The nature of a ballot means that maybe I'm just unlucky, but the perception that I take away from chats with my friends - some of whom also appear to be as unlucky as I am - is that it seems more difficult now to successfully ballot for a BTO flat than before. 

Is there publicly available data that we can use to check this? Well, kind of. 
