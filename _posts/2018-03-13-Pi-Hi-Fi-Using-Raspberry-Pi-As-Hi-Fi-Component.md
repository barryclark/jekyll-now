...
RuneAudio is capable of streaming media from a network drive, but I wanted a totally wired connection, so I used a high capacity USB memory stick (128GB) plugged directly into the Pi with all my music collection on it. If you want to update your collection later, you can simply FTP files to the USB stick via the Pi (use SFTP connection with local IP address of your Pi eg 192.168.0.xxx, log on as "root", and use /mnt/MPD/USB/MUSIC as the default remote directory)
