...
| Readable      | A human familiar with the domain can read the test name and test body (even non-programmers) and understand why this test case matters. | Code under test must use in-domain names at the right level of detail. |
| Organized     | If I wonder "is there a test for the for this requirement?" I can easily find the corresponding test case / see that it does not exist. | May break the 1 test :: 1 class relationship |
