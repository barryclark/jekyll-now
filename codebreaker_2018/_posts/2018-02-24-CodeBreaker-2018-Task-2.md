...
For this task we are asked to find the secret key required for generating the OPT the ransomware. To find the secret key the _cid function needs to be analyzed further. The time-based one time password (TOTP) is defined by [RFC 4226](https://tools.ietf.org/html/rfc4226). The task asks for the secret to be submitted base32 encoded. 
