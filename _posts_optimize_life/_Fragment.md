...
반복되는 부분을 쉽게 고칠 수 있는 방법은 없는가?
카테고리를 어떻게 나눠야 할지 생각해야함.
하지만 빨리 만들기를 원함... 조금더 빨리 만들어야 하나..?
변경하기 편하게 작성하기, 디버깅 하기 편하게 하기
DU1, DU2 애니메이션을 1초로 잡아놓고, 몇 초동안 인지 설정하면 편하다
매몰되면 해결 방법을 앞에 두고도 못찾음
지금 테크트리 쌓는 단계, 빡세게 공부하는 것이 좋다.
SVN은 이상하므로 정말 신중하게 올리기
자기가 변경한 것만 올리기, 충돌나므로
자기가 뭘 변경하는지 항상 말하기
테스트 폴더 만들어서 안올리던지.
변경되었다고 알려주면 Update하기
1. 커밋의 기준은, 플레이할때 치명적이다 하면 바로커밋,
아니라면 집갈때 한번에 커밋하는 방향으로.. 같이 작업하기 때문에...
리다리렉터도 못 고치네..
업데이트 함부로 하면 덮어씌우므로, 조심해서 하기.
삭제좀 조심해서 하자. 습관적으로 삭제하는 버릇이 오류를 불러온다
기획이 이상해도, 팀워크가 맞으면 결과물이 나올 수 있음.
구글 드라이브 볼때는 무조건 다운받아서.
글이 너무 많으니 놓친다. 단순하게 만들어야 한다..?

발전시키기
코드 정리하고 싶다...
1. 위젯은 에넘으로 상태 관리하고, 근데 언제??
어떨때 에넘을 쓰는 것이 좋지..?
그냥 전체적으로 Enum을 쓰는게 언제 편한지 알 필요가 있다.
다만, 블루프린트 에넘을 변경하면, 이를 이용하는 다른 블루프린트들이 변경됩니다.
2. 활성화 비활성화 코드는 하나로 합치고, 불변수 파라메터 넘기고...
스크린 액터들
1. 베이스로 합칠 수 있는 것 합치기. 
2. 또한 스크린 액터 베이스 으떻게 해야,, 상호작용을 깔끔하게 관리할 수 있을까..? 생각해보기
2. 스크린들 콜리전이랑 하이라이트 설정, 하나로 합치기.
손가락 캡슐콜리전으로 해서, 좀더 누르기 편하게 만들자. 구체로 하닌까 가끔 통과해서 불편하네.
15-8 외부등 불켜기, 추가하기...? 그것때문에 배경을 어둡게 했다가, 밝게 했다가... 요청하면은, 바꿀필요는 없을 듯 하다.

클래스 공통 부분 좀더 고려 (위젯들을 공통으로 묶을 수 있을까에 대한 고민)
희한한 변수명에 주석 또는 툴팁달기
VR은 느낌이라, 기능은 자동화 테스트를 할 수 있을지 몰라도, 느낌이라 결국 직접 해야 한다.

애니메이션을 사용하면 하드코딩 되는 경우가 있으닌까, 변수로 편하게 바꿀 수 있도록 하는 것이 좋다. 이런 판단들을 하면 좋다.
