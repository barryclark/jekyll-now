---
layout: post
title: C# sharp
---

{% include _shared_block/_collection/CSharp_StudySite.md %}

## Use Class
### Extension Method

{% include _shared_block/_cSharp/CSharp_ExtensionMethod.md %}

### Hide Constructor

{% include _shared_block/_cSharp/CSharp_HideConstructor.md %}

## Attribute

{% include _shared_block/_cSharp/CSharp_Attribute.md %}

## Activator

{% include _shared_block/_cSharp/CSharp_Activator.md %}

{% include _shared_block/_cSharp/CSharp_Activator_CreateInstance.md %}