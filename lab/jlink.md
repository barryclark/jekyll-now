...
When invoking a function, the Fn infrastructure will get the request via a trigger (ex. HTTP) and will start the container for that function using the appropriate function container (assuming it isn't alerady running).
...
If you look at the content of the newly created "modularfunc" directory, you will see familiar content (`pom.xml`, `func.yaml`, `HelloFunction.java`, etc.); the only noticeable difference being a new `Dockerfile`.
