...
The Java Platform Module System (JPMS), introduced in Java 9, adds [modularity to the Java platform](https://blogs.oracle.com/java/modular-development). With JPMS's `jlink` it is now possible to create a Java Runtime that only carries the modules that are required by your application.
...
`fn init --init-image delabassee/jlink-jre modularfunc`
