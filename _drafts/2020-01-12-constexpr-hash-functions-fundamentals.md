...
As the first hash function I chose the simple [djb2 hash function](http://www.cse.yorku.ca/~oz/hash.html) in the XOR variant. It is a very simple algorithm, nevertheless it has some [practical uses](https://github.com/hashcat/hashcat/issues/1925). Let us have a look at the C implementation:
...
Let us see how we can transform it into a ˋconstexprˋ function that we can execute at compile time or at runtime first in C++ 17 and later on in C++ 11.
...
And that's the story if you use C++ 17. If either you are interested in how this could work in C++ 11 or if you are into functional programming and cannot stand ˋforˋ loops then keep reading.

# Going Functional and Making It Work for C++ 11

To make the code from above work for C++ 11 we have to do two things: 

1. We must find an alternative to ˋstd:string_viewˋ because it is only available from C++ 17 onwards.
2. We have to work around the limitations of ˋconstexprˋ in C++ 11 compared to C++ 17.

The first problem is easily solved by using ˋboost::string_viewˋ from the boost/utility library. So the function signature becomes:

ˋˋˋC++
unsigned long constexpr djb2_xor(const std::string_view & str);
ˋˋˋ

The second problem is more interesting, because the amount of stuff a constexpr function body is allowed to contain is so limited that I can [cite](https://en.cppreference.com/w/cpp/language/constexpr) it here in its entirety:
...
All we need to do is to calculate the new hash from the previous value ˋhashˋ and the character at the given index ˋidxˋ. Then we feed this hash into the next call with the same string argument and the position argument ˋidx+1ˋ. The recursion terminates when the index is outside the boundaries of the string. There is only one more thing we have to account for. We cannot have an if statement in a C++ 11 ˋconstexprˋ function. We can solve this problem using the ternary operator ˋ? :ˋ. Finally we have:
