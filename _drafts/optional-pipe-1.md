...
The operator returns  ``nullopt`` if the optional argument was empty. If the given optional is not empty, the callable is applied to the value inside the argument and returns an optional containing the result. So the signature of the operator is ``operator|: const std::optional<T> &, F&&``$$\rightarrow$$``std::optional<U>``, where ``U`` is the return type of the callable. This operator is already quite neat to use because it can be chained. Let's look at an example.
...
```
We arrive at the desired result y chaining the `std::string::size` member function and a generic lambda that squares its argument. The result is itself an optional of type `size_t`. It contains a value only if the optional of type string was not empty. The pipe syntax allows for a neat way to chain operations on optional values. It relieves us of the responsibility of checking whether the intermediate results contain values before operating on them. However, there is (at least) one major issue with the implementation. It has to do with the return type.
