...
ESC누르면 열려있는 UI가 닫히는 것도 만들어야 한다...
...
[Let’s create an equipment inventory in UE4](https://medium.com/geekculture/lets-create-an-equipment-inventory-in-ue4-456c79aa1247)

배열에 인덱스로 접근하려면 어떠한 일이 일어나나?

장점으로는
1. 인벤토리를 직접 몰라도 된다. 완전히 독립된 상태를 유지할 수 있다. 자료형이 Int형 이기 때문에,

포인터로 접근하면 어떠한 일이 일어나나?

인벤토리가 알아야 한다. 인벤토리 위젯을...?

아이템을 추가할 때, 성공 실패여부가 있나?

빈 아이템 클래스를 사용하는 이유는 뭘까,, nullptr을 쓰지않는 이유는..?


## 인벤토리의 경우, 몬스터, 캐릭터가 가지고 있는게 편하다.
왜? 몬스터도 인벤토리를 가지고 있기 때문, 죽을 때 떨구는 용도로...

## 언리얼 게임 매니저

## UI 통합 환경을 만들어야함.

1. SetInputModeGameAndUI 메서드를 이용, 항상 화면에 잠금되도록 해주고,
1. Cursor위치를 화면 가운데로 옮기고, 
2. 커서를 보이게 합니다.

* 위젯이 한 화면에 2개 이상이 되면, 순서가 필요해 지므로, 한번에 하나씩 켜지도록 할 수 있습니다.
...
Initialize가 있으면, Deinitialize가 있어야 하고,
Construct을 만들면, Destruction이 있어야하 하고,
Begin이 있으면, End도 있어야 한다.

상태가 해제되었는지 표시해 주는게 없기 때문에...
...
패키징할때, 한글경로 한개라도 있으면 무조건 깨진다.

만들 때, 제일 힘든것. Shipping일 때는 터지고, Debug는 되고, 메모리 인지를 잘못한 것이다...

기획에 관련된 것인데. 마우스가 평상시에 보이는 경우, 위젯을 여러개 열 수도 있지만, 마우스가 평상시에 안보이는 게임인 경우, 마우스가 보이면 안된다.

플레이어 Info 위젯에 들어가보면, Construct는 Construct함수가 아니다. 근데 OnInitialize는??? 생성시에 불린다..?
...
## 컴포넌트 사용법을 알아야 할때,
위아래에 특정 컴포넌트를 붙여야 되거나 하는 경우, 액터로 필요한 컴포넌트를 묶어, 완성된 형태로 관리하는 방법이 있다.
...
**점프 루트모션이 적용이 안된다. 흠...**
해결방안은. 안되는 이유가, 캐릭터 무브먼트에서, 설정과 연관있는데, 세팅을 Fly로 바꿔줘야 한다. 노티파이로 설정하면, 극도로 직관적..?

지면에 붙이는 시스템 때문에 그런 것 같은데, 암튼.

루트모션의 경우, 블렌드 타임을 짧게 잡아야 지연되는 느낌이 없을것이다.

점프하면서 이동하기 위해서는, 삼등분 해서, 애니메이션을 적용해야 한다.

점프 끝나는 애니메이션은 최대한 짧아야 한다.

Frank Dual애니메이션을 이용해 애니메이션을 만들어 볼 수 있습니다. 이를 보면 왜 Mixamo를 쓰지 말라고 하는지 알 수 있습니다.

* 개인적으로는 둘다 크게 마음에 안듭니다.

점프같은 경우, 신비를 보고 따라 만들어 봐라.

**게임상에서 동작에 관련된 것들은 대부분 루트 모션이다**
제일 만들기 어렵고 드러운게 점프이다.

## 게임에서 애니메이션,
게임에서 애니메이션은 과장되어 있음. 만화랑 비슷하다 생각하면 됨. 이러면 안된다라고 삼아도 될 것 같은데, 열혈강호 모바일 생각하자.

**가벼운 무기는 바로 모션을 주면, 가벼운 느낌을 줌. 무거운 무기는 가운데, 지연을 두면, 무겁다는 느낌을 줌. 중간에 모션이 없으면, 굉장히 지루한 느낌을 줌.**

사소하긴 한데, 중요한 재미 요소지이?

일정한 속도로 만들면, 디렉팅에서 OK 안떨어 진다. 처음부터 끝까지 속도가 똑같으므로, 심각하게 재미가 없다. 되게 못만든 애니메이션이다.

* 애니메이션을 잘 만들고 싶어도, 현재 내가 경험이 없으닌까, 차분히 하도록 하자.

* Mixamo Hm...

* blender에서 Mixamo를 unreal용으로 바꿀 수 있음?!? 인터넷에 다 나오기는 함.

* 재미를 위한 미묘한 디테일인가?!?

* Mixamo converter도 다운받아야함. 압축을 풀면 안됨?
* Mixamo converter 설치해서, 오른쪽 쓰윽 잡아땅기면 보임

* 밑에 프리퍼렌스 / 애드온 / 

* 애니메이션 가져올때, DefaultSampleRate를 반드시 키도록 한다. 20에서 30Frame으로 가는건 상관 없는데, 3000짜리 하나 있으면, 감당 안된다.

## 흠..
흠... 빙의가 안되는 이유,

실제 모든 게임은, 시작하자마자 플레이 되는 게임이 없음. 유저가 의지를 가지고 시작하겠다 해야 시작한다. UI가 되었건, 인게임 인트로가 되었건, 게임 플레이가 되는 화면이 아니라, 유저가 시작하기 전에 보여주는 화면이 하나 있고, 그다음 시작해야 시작한다. 모통 그때 레벨전환을 시키면 게임모드를 사용할 수 없다.

일딴 뭐가 문제였냐면, AI controller는 중간에 못 바꿈. 이를 바꿀려면, AISpawn으로 한다. 음...

결론은 AI가 null로 들어간다. 음. 여기다 함수 하나만 불러주면 된다. SpawnDefaultController를 호출하냐 안하느냐의 차이이다.

* 문제를 명확하게 파악하자, 효율적인 해결법이 나온다? 분명히 사소한거 하나때문에 그렇다?!?

## branch에서 issue를 종료시킬 경우, pull request했을 때 적용된다.

종료시키는 방법에 대해서는 https://www.hahwul.com/2018/07/27/closing-git-issue-with-commit/ 를 보도록 하자.

## 프로그래머에게 논리란 엄청 중요하다?

? 마이크로 소프트 메이크 파일이란 무엇인가?

c++ 사용하지 않는 매크로 표시
[사용하지 않음](https://stackoverflow.com/questions/295120/c-mark-as-deprecated)

* CSV로 테스트 케이스 추가, 생각해봅시다.
    - https://dev.epicgames.com/community/learning/courses/19G/unreal-engine-c/bLmd/unreal-engine-csv-json
    - https://wergia.tistory.com/154

[마인드](https://velog.io/@gunilna/%EC%8B%A4%EC%9A%A9%EC%A3%BC%EC%9D%98-%ED%94%84%EB%A1%9C%EA%B7%B8%EB%9E%98%EB%A8%B8%EB%A5%BC-%EC%9D%BD%EA%B3%A0)

1. 탄막 알고리즘
[알고리즘이란 무엇인가?](https://m.blog.naver.com/PostView.naver?isHttpsRedirect=true&blogId=foat3376&logNo=70167825094)

2. 코드 디자인 패턴 ( ex: 싱글턴, 상속, 추상화... )

개발자 세계에는 수십 개가 넘는 디자인 패턴이 있습니다. 사실 어떤 문제에 어떤 패턴을 사용해야 할지 판단하는 것은 참 어려운 일입니다. 특히 디자인 패턴에 대해 빠삭하게 알고 있지 않은 개발자라면 더욱 어려울 것입니다.

[디자인 패턴](https://readystory.tistory.com/114)

3. 데이터 관리 ( CSV, JSON 등을 이용한 데이터 드라이븐 방식의 개발 )

4. 코드 가독성 관리 ( 네이밍 규칙, 코드 구조화등..)

* [코드 구조화](https://han7096.medium.com/%EC%A2%8B%EC%9D%80-%EA%B0%9C%EB%B0%9C%EC%9E%90%EB%A1%9C-%EA%B0%80%EB%8A%94%EA%B8%B8-%EA%B5%AC%EC%A1%B0%ED%99%94%EB%90%9C-%EC%BD%94%EB%93%9C-5d03116dc78b)를 간단히 말하면, 나누면 알아보기 쉽고, 재사용 하기 쉽다.

5. 최적화 ( 상황에 알맞게 컨테이너(ex: 큐, 리스트..)를 사용했는지, 텍스처, UI, 충돌처리..)

위와 같은 항목들에 대해서 고민해보고, 관련 서적들도 들춰보면서 개발해보는게 중요해보입니다.

그리고 이렇게 개발한 내용을 가지고 잘 정리해서 일목요연하게 문서를 작성하는건 당연하겠죠?

##
언리얼 만들 떄 find같이 오버헤드 정신나간거는 안쓴다.

게임 매니저,

레벨 전환

게임 인스턴스로 유지되어야 하는 값들 유지하기

프로그래밍에서 관계를 맺는게 제일 힘들다. (이가 누가 누구를 아는가)

레벨 블루프린트는 이 관계를 싹다 무시하게 된다. 스포이드로 찍어서 블루프린트 통신하는 것도 레벨 블루프린트랑 다를 게없다.

테스트 코드를 확인하고 싶으면, 레벨 블루프린트에 만들어도 되는데, 게임 코드는 절 때 레벨 블루프린트에 만들지 말아라.

레벨에 배치된 물건 가지고 레벨 블루프린트로 만든다. 이딴 식으로 만들면 안된다. 액터를 만들고, 액터에서 관리하게 만들어야 한다.

마우스 우클릭, 콜리전의 비긴 오버랩, 마우스 우클릭, 바위에 대한 레퍼런스 생성, 액터 오브 오브 로케이션을 얻어옴.

레벨 블루프린트에다 작업하면 관리가 안된다. 테스트 ㅏ할 때는 편할지 몰라도, 절때로 레벨 블루프린트에 작업하지 말아라.

레벨 블루프린트는 입력도 받는다.

## 게임 모드,

* 사람이 입력하는 입력체계를 컨트롤러라고 한다.

* 왜 분류하지 않냐 하면, 게임 내에는, 컨트롤러가 폰에 붙어있기 때문에, 하나인 식이다.

* AI컨트롤러가 있고, 플레이어 컨트롤러가 있는데, 폰에 어떤 것을 붙이냐에 따라, 나눠질 수 있다.

* 플레이어 컨트롤러는 딱 하나라고 생각하자.

* 컨트롤러가 여러가지가 있어야 된다면, 미니게임이 있고 극단적으로 달라지는 경우가 아니라면, 대체로 하나일 것이다.

* 컨트롤러라는게 대단한 게 아니라, 키보드 입력과, 마우스 입력만 빼내면 됨. 

* 플레이어 컨트롤러는 강제로 붙이는걸 빙의라고 한다.

* 왜 굳이 이렇게 했냐는 것이쥐,

* 꺼내놓으면, 직접 블루프린트 통신하는 방법이 있지만(스포이너로 직접 찍어서 캐릭터를 선택하는 것), 작은 게임이 아니라면 콜리전으로 알아야쥐,

* 게임 내에서 플레이어 컨트롤러를 갈아끼울 수는 있어도, 여러개를 입력시킬 수는 없음.

* 입력의 차이가 무엇인가? 월드에 배치되어 있는 상태와, 스폰후 폰에 배치하는 상태에서,

* FPS는 빙의, RTS는 카메라만 컨트롤하고, 싹다 인공지능에게 명령

* 코딩 구현을 보면, 왜 캐릭터 인풋에서, 피치와 요를 분리했는가?
    - 시뮬레이션 같이, 플레이어가 있으면, 
    - 산이 있을 때, 빨라야 하나, 느려야 하나? 2.5d 게임에서, 산을 오를 때 더 빠른 이유,

* 게임의 대부분은 콜리전이다. 명심하자아?

빙의 하는 순간, 캐릭터를 다르게 할 수 있다.아?
빙의가 넘어가면 어떻게 되는가?

빙의시 인풋이 어떻게 변경되는가? 이거는 코드보면 답 나오겠네,

빙의 되는 순간, AI 컨트롤러는 기본적으로 붙어있고, 빙의가 풀리는 순간, 기본 컨트롤러로 넘어간다.

AI 컨트롤러가 없다,? 왜 없냐? 그것도 문제네.

기본으로 들어가 있는 AIController가 있는데, 빙의가 끝났는데, AI가 연결이 안되어 있다?.

근데 물리족 무브먼트에 틱이 비활성화 되어 있다. 흠... 


## 메서드의 선언
메서드는 메서드를 실행하는 주체가 가지고 있게 하자.

## 게임 오브젝트 매니저
게임 오브젝트 매니저는 게임 오브젝트를 관리하는 방법으로, 게임 오브젝트를 관리하려면 씬에 올려진 게임 오브젝트들을 보관하고 있어야 합니다. 미리 올려진 오브젝트인가, 실시간 오브젝트인가에 따라 시작이 달라집니다.



## 2022 07 20

* 데미지 계산할 때 float인데, 소수점, 더하기 빼기, 자료형 절대로 실 수 하지 말아라. 신뢰도가 팍팍떨어진다.

웨폰과 몬스터, Health Tag

언리얼에서 객체를 다루는 가장 기본적인 객체가 오브젝트 클래스,

언리얼 명령어 중에, 서브 클래스 어쩌구 있는데, 블루프린트에서 안됬던 것으로 기억하는데.

오버라이딩 되어 있는 것도 없고, 언리얼에서 객체를 관리하는 가장 작은 단위이다.

모른다. 어떤 스텟을 어떤 것을 상속받아서 하건, 알아서 해라.

오브젝트를 가져다가 생성하는 시점은 크리에이트 밖에 없다.

유 오브젝트를 뭐, 구조체 몸통에 애 하나만 바꾸고 싶을때 어려움.. 구조체 내용을 갖다가 하나만 바꿀라면, 무슨짓을 해야 하는가?
**구조체의 멤버설정을 항상 기억하자. 핀 옵션으로 부분적으로 바꿀 수 있다.**

많이 쪼개면 쪼개질 수록, 어떻게 관리해야 하나, 관리를 위한 클래스를 또 만듬. 근데, 역활분담 잘 되어 있어서 관리를 하게 되면, 코드의 유연함을 설계적 조립으로 가져갈 수 있다.

* 잘라놓고 코딩한 애들은 전체적인 코드를 볼 수 있으나, 묶어 놓고 코드하는 애들은, 잘라놓은 코드를 못 알아봄.

유지보수 할 때도 그렇고, 

프로그램이 모든 환경이 갖춰저 있고, 구현하는 건 쉬움, 그 걸 위한 게 어렵다.

MMORPG를 만들 때 오브젝트를 이용해서, 폰에서 받아오는 것, 한 5배 정도밖에 안된다.

여러분이 회사에서 제대로된 실력이 있는 프로그래머는 10명중 1명꼴이다?

MMORPG만들때 10~20명인데 그중에 프로그래머라 부를 수 있는 애들은 3명정도다.

특히 모바일로 넘어가면서 코드 난이도 떨어졌다? 다 엔진에서 써있는...

특히 c#에서는 메모리 관리가 없음?!?

나중에 포폴을 낼 때, 어떤 식으로 프로그램을 만들어서 반영을 했는가? 최소한 선 하나라도 그릴 수 있다?

## 

* 아무래도 C++로 코드 작성하는 것이 압도적으로 효율적입니다. 그래서 블루프린트로 통신하는 방법은 일일이 구현하거나 따로 빼도록 합시다.
...
## 2022 07 19
전제가 보통 무기는 하나만 붙는다는 것을 전제로 게임을 만든다.

한개짜리 무기 슬랏 시스템은 (이도류 같은 것을 의도하지 않은) 칼 두개짜리 한 개 아이템을 만듭니다.

무기 슬랏 한개가 있고,
1. 무기를 장착하는 것은 아이템을 스폰하고 인벤토리에 붙임을 의미합니다.
2. 무기의 각도는 수직으로 붙어야 합니다. 소켓을 이용하여 조절합니다.
    - 신비의 캐릭터를 보면, 손모양에서 직각으로 붙여집니다.

* 캐릭터가 죽고 무기만 있는 경우는 없습니다.

공격 판정의 경우, 
1. 피격받을 대상에게 캡슐을 추가해 줍니다.
2. Actor에 Monster라 Tag를 추가합니다.
3. 제너레이트 오버랩 이벤트를 껐다 키는 식으로 작업합니다.
    - 피직스가 메세지 처리하는 식으로 작업하는데, 
//3. AnimNotifyState로 무기의 콜리전을 켰다 꺼줍니다.
4. 데미지가 중복해서 들어가지 않게 해야합니다.
    - 고유 이름을 가져와서 처리합니다. 이는 한 오브젝트에 콜리전이 여러개일때 사용할 수 있습니다.
    - 스트링이니, 세트를 이용하는 것 같습니다.
    - 다만 액터의 포인터를 이용하지 않는 이유는 모르겠습니다.
5. 공격이 끝났다면, 비워야 합니다.

* 이름정리를 제대로 안하면, Attack, Hit 맞은 놈이랑 헷갈릴 수 있음.
* 회사에서 코딩하다 보면, 오만가지 상황이 발생하는데, 추가하는 것을 먼저 하고, 작업합니다.
* 버그를 고려할 떄, 다단 히트보다, 아예 히트가 안되게 작성하는 것이 좋습니다.
    - 다단 히트면 유저가 조용히 있습니다.

* 메세지 처리하는 엔진을 사용할 때는, 잡기가 힘들고 인지가 힘듬.
    - 되도록이면 콜리전 Enable을 조절하지 마라,

* 무기가 있는지 없는지 검사하는데(IsVaild), 디폴트 무기가 있는(왼손) 상태를 보장해서, 검사하지 않는 식으로 만들어야 합니다.
    - 지금은 리소스 관리할 때, 디폴트 리소스를 관리하는 식으로 정리하는 방향입니다.

* C언어로 코딩할 때 특정객체의 고유 아이디를 저장하면, 포인터를 저장하면 됩니다.

* 코딩이 문제가 아니라, 어떻게 관리할 것인가가 관건. 구현에 목메달면, 관리가 안됨. 코드는 밖에서 부터 안으로 들어감.

##
Weapon을 추가하고,

* 게임을 만들 때, 보장받는 다는 것이 중요합니다. 이는 버그를 내지 않기 위해서 입니다.
    - 이해하기 쉽게, 수학에서 공리라고 이해 할 수 있습니다.
    - 객체가 죽으면 포인터는 어떻게 할 것인가? 의 스마트 포인터도 생각할 수 있습니다.
    
## 2022 07 18

여기서 무기를 추가함.
무기마다 데미지가 다르고, 무기마다 공격판정이 다름.

1. 무기르 손에 붙여야 하고, 무기가 애니메이션을 따라 움직여야 하고, 무기에 있는 기능은 무기에 있어야 하고, 공격판정은 애니메이션에 있고, 캐릭터가 무기를 들고 있기 때문에, 애니메이션에서 직접 엑세스 하는게 아니래 캐릭터를 통과해서 전달할 것임.
    - 다만 무기는 스켈레탈 메쉬로 되어있다, 왜 스켈레탈 메쉬를 이용했나? 하면, 예전에는 스테틱 메쉬에서 소켓을 이용하지 못한다. 그래서 스켈레탈 메쉬로 했었던 것인가.

지금은 무기 몇가지 안할 거닌까, 시키는 대로 할꺼니까, 

콜리전과 소켓을 붙입니다.

여기에 스태틱 메쉬를 두는게 아니라 엑터를 두고, 씬 컴포넌터를 둔다음 붙이는 식으로 작업이 이루어진다.

* 스테틱 메시에 시작할때 NoCollision으로 설정한다고 하자, 근데, 웃기는건 저기서 버그가 날 수 있음.
    - 제일 처음 생성하고 공격판정이 한 프레임 호출됬다가 꺼질 수 있음.
    - 메세지 시스템은 이런 것들도 고려해야 할 것임.
    - 한 프레임 오차로 인한 문제들은 경험적으로 다 겪지 않으면, 모름.
    - 따라서 비긴 스타트 컨스트럭트에 있는 것이 맞음.

- 이벤트에서 True면 켜주고, False면 꺼주는 식으로 만들자.

주인공이 하나인 경우, 플레이어 스텟을 써도 되는데,,,

애니메이션을 다시 배치할 때, Loop Animation을 켰는지 아닌지 확인 필요하다.

1. 애니메이션에서 노티피 발생
2. 노티피에서 서드퍼슨으로 명령 발생
3. 서드퍼슨에서 웨폰으로 명령 호출
4. 오버랩 해서 사건 발생했어 알려줌
    - 여기서 다시 거꾸로 진행하는 것이 어려움.
    - 따라서 이벤트 디스패치를 사용하여, 처리해야함.
    - 무기에 이벤트 디스패치를 두면, 무기가 캐릭터를 알 필요가 없다?.

##

* 설명이 없다는 것은, 설명할 필요가 없거나, 거의 쓰지 않는다는 것. 대부분 앞의 이유임. 한마디로 중요한 개념을 모르고 있어서 사용하지 못함을 의미한다.

* FKey 구조체는 #include "InputCoreTypes.h"에 정의되어 있고, InputCore에 속합니다...

* GetInputAnalogKeyStat메서드로 Key input(키 상태)를 가져올 수 있습니다. PlayerController에 있습니다.

Actor의 Tick 관련
http://egloos.zum.com/sweeper/v/3220906

## PrimaryComponentTick

생성자에서 PrimaryComponentTick.bCaneverTick = true 는 틱을 사용함을 의미합니다.
Tick의 키고 끄는 것은 SetComponentTickEnabled(bool)로 가능합니다.

	PrimaryComponentTick.bCanEverTick = true; 해줘야 하고,
    
	SetComponentTickEnabled(false);를 비긴에서 꺼준 후,

    마우스 입력에 따라 껏다 킨다.

## 2022 07 15

신비를 빼껴서, 상하체 분리 애니메이션을 만든다.
...
안그래도 AnimState로 만들었는데, 여기서 몽타주를 추가하지 않고 애님스테이트로 할라면, 애님스테이트의 상태를 백업해둬야함. 충돌모션을 재생할 수 있는지, 등등의

그래서 PlayerState라고 따로 있기는 함. 다만 여기서의 문제는 GameMode에서 설정한다는 것이 문제,,

한글로 몽타주 치면 몽타주 재생나오고, 영어로 Mongtague쓰면, 재생 스탑등등이 나옴. 한글로 되어있는 것을 써야할까? 영어로 되어있는 것을 써야할까? 언제 끝나는지 알아야할 필요가 있는지, 아닌지에 따라 결정됨.

몽타주 디폴트 슬롯의 의미는, 모든 몽타주는, 슬롯을 가짐.

슬롯이 있고, 그룹이 있음??? 아무때나 총을 못쏘게 만들어야 한다. 그래서 상태체크가 필요, 음. 

딱 기본공격 정도만 상하체를 분리하고, 기본 동작같은건 집어넣지 않음. 그래야 이쁘게 나오기 때문에,

애니메이션에 어디서부터 섞고, 어디까지 섞지 않을지, 애니메이션에 표기를 해줘야함.

비긴오버랩, 앤드오버랩도 마찬가지인데, 뭐하건 반드시 앤드 오버랩을 보장해줌. 애님 노티파이 스테이트도 애니메이션이 중간에 끈어진다 하더라도, 반드시 종료를 보여줌.
