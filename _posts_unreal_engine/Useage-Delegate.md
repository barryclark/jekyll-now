...
* 멀티 케스트는 싱글 케스트와 동일하지만 여러 함수를 바인드할 수 있습니다.
* 이벤트는 멀티 케스트와 동일하지만 전역으로 설정할 수 없어 외부 클래스에서 추가 델리게이트 선언이 불가능합니다.
* 다이나믹은 싱글과, 멀티 두개다 존재하며 다이나믹 델리게이는 직렬화(serialize)화 되어 블루프린트에서 사용 가능합니다.

* ...\Engine\Source\Runtime\Core\Public\Delegates\DelegateCombinations.h 에서 선언을 확인 할 수 있습니다.

? 리턴값이 있는 델리게이트 어쩌라는 건지 모르겠다. 마지막 값만 리턴되는 것 같기는 한데? 직접 사용해봐야 알 수 있을 것 같다. 아니면 리턴값이 있는 델리게이트는, 한개만 하나만 바인드된다던지 하는건가?
...
* 콜백을 지정하는 Delegate를 사용할 때 주의사항입니다.
...
? 도대체 반환값이 있는 델리게이트는 어떻게 사용하는 겁니까?
