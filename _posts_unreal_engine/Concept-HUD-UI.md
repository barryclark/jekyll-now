...
- [ ] UProperty bind 이용하기 외 내용 정리하고 다듬기
- [ ] Slate 음... 어떻게 써야 하나?
- [ ] 3D Widget을 사용하는 방법
...
UMG로 만드는 것을 전제로 사용하기 쉽게 변수와 메서드를 생성합니다. 애셋은 UserWidget클래스는 블루프린트에서 상속받아 설정합니다. **단 UMG를 사용할 때 애니메이션은 유용하긴 해도 비용이 높아서 조심해서 사용해야 한다고 합니다.**

<details><summary>그렇게 하는 이유</summary><div>
...
[HUD는 오래되긴 했지만, 여전히 디버그를 목적으로 사용하는데는 유효하다고 합니다.](https://forums.unrealengine.com/t/why-have-a-hud-class/55660/4) 

UI 예산이 마치 0이나 다름없는 HUD와 같이 퍼포먼스가 중요한 곳에 사용될 수 있다고 합니다.

* 개인적인 판단을 보류중... 오래된 내용이라 정확하지 않은듯함.

<details><summary>HUD로 Debug한다면</summary>
<div markdown="1">
...
## 프로퍼티 바인딩을 이용해서 변화를 적용하기

## WidgetComponent이용하기

왜 사용해야 하는가.
1. 3D 공간이기 때문에, 조금만 멀어져도 크기가 굉장히 작아집니다.
2. Backspace culling이 적용되어, 뒤에서 보면, 보이지 않습니다.
3. Backspace culling을 막는다고 해도, 글자가 뒤집혀 보입니다.
...
<details><summary>어떻게 하라는 것인가?</summary>
<div markdown="1">
...
먼저 두 함수 모두 UUserWidget에서 정의됩니다.
