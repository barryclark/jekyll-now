...
# sync all css
aws s3 sync $SITE_FOLDER s3://$BUCKET_NAME/ --exclude '*.*' --content-encoding 'gzip' --cache-control 'max-age=6048000, must-revalidate' --include '*.css' --content-type 'text/css'

# snyc all javascript
aws s3 sync $SITE_FOLDER s3://$BUCKET_NAME/ --exclude '*.*' --content-encoding 'gzip' --cache-control 'max-age=6048000' --include '*.js' --content-type 'application/javascript'

# sync feed xml
aws s3 sync $SITE_FOLDER s3://$BUCKET_NAME/ --exclude '*.*' --content-encoding 'gzip' --cache-control 'max-age=7200, must-revalidate' --include '*feed.xml' --content-type 'application/rss+xml'

# sync sitemap
aws s3 sync $SITE_FOLDER s3://$BUCKET_NAME/ --exclude '*.*' --content-encoding 'gzip' --cache-control 'max-age=7200, must-revalidate' --include '*sitemap.xml' --content-type 'application/xml'

# snyc html
aws s3 sync $SITE_FOLDER s3://$BUCKET_NAME/ --exclude '*.*' --content-encoding 'gzip' --cache-control 'max-age=7200, must-revalidate' --include '*.html' --include '*.htm' --content-type 'text/html'

# snyc txt
aws s3 sync $SITE_FOLDER s3://$BUCKET_NAME/ --exclude '*.*' --content-encoding 'gzip' --cache-control 'max-age=86400' --include '*.txt' --content-type 'text/plain'
