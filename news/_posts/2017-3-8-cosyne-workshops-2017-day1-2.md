...
1. *The things that neurons respond to are straightforward to guess.*
	If you believe that regions of the brain are compartmentalized based upon function, then you would live and die by that assumption. But the realization that the brain works in concert with many of its parts simultaneously is nearly pervasive in neuroscience these days. Konrad called out just a few examples that make it hard to interpret tuning curves as being functionally relevant for only one or two things. Examples: Motor neurons can actually respond to [reward outcomes]( http://journals.plos.org/plosone/article?id=10.1371/journal.pone.0160851), and expectation of a later event occurring can be encoded in the [visual cortex](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4635676/).
...
Most things are just easier to deal with if they have a linear response, so it’s a good regime to work in for analysis reasons. But if the response to behaviors or stimuli are nonlinear, this can absolutely be a dangerous assumption to make. Konrad’s main example here was that V4 neurons that encode color are quite nonlinear with respect to changes in the types of scenes that are shown.

3. *The brain processes and hands off information to different functional regions in a step-by-step process.* 
I didn’t catch a ton on this topic, but he did pull up a schematic for functional connections in the brain from his and Eric Jonas’ [recent article](http://journals.plos.org/ploscompbiol/article?id=10.1371/journal.pcbi.1005268): 
<p style = "text-align: center;"> 
<img src="/images/kording_brain.png" width="400">
</p>
(Functional areas of the brain, and their connections as we currently know them. Adapted from the [PLOS Comp. Bio. paper](http://journals.plos.org/ploscompbiol/article?id=10.1371/journal.pcbi.1005268))
