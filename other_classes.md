---
layout: page
title: Other Classes
permalink: /about/other_classes/
---

Misc classes I plan to take when I have time.

### Astrophysics Xseries

| Class | Completed | Repository |
| ----- | --------- | ---------- |
| Greatest Unsolved Mysteries of the Universe | In progress | [repo](https://github.com/mattboler/astro1x)|
| Astrophysics: The Violent Universe | | |
| Astrophysics: Cosmology | | |
| Astrophysics: Exploring Exoplanets | | |

### Aerospace

| Class | Completed | Repository |
| ----- | --------- | ---------- |
| Introduction to Aerospace Engineering | In progress | [repo](https://github.com/mattboler/introaero) | 

### Robotics Micromasters

| Class | Completed | Repository |
| ----- | --------- | ---------- |
| Robotics: Fundamentals | | |
| Robotics: Vision Intelligence and Machine Learning | | |
| Robotics: Dynamics and Control | | |
| Robotics: Locomotion Engineering | | |

### BioInformatics (If I ever have time)
Classes from [OSS - BioInformatics](https://github.com/open-source-society/bioinformatics)

[![Open Source Society University - BioInformatics](https://img.shields.io/badge/OSSU-bioinformatics-blue.svg)](https://github.com/open-source-society/bioinformatics)

* Classes where notes or homework are done primarily on paper will be scanned or converted to TeX and uploaded to repos as I find time, but they will lag behind programming classes in terms of being up to date.

#### 1st Year

| Class | Completed | Repository |
| ----- | --------- | ---------- |
| Introduction to Biology | ON HOLD | | 
| Principles of Chemical Science | | |
| CS 1 - Python 1 | | |
| CS 1 - Python 2 | | |
| CS 1 - Principles of Computing 1 | | |
| CS 1 - Principles of Computing 2 | | |
| Calculus 1 | Covered by Math 1610 @ Auburn | |
| Mathematics for CS | | |

#### 2nd Year

| Class | Completed | Repository |
| ----- | --------- | ---------- |
| Biochemistry | | |
| Organic Chemistry | | |
| CS 2 - Object Oriented Java | | |
| Calculus 2 | Covered by Math 1620 @ Auburn | |
| Introduction to Databases | | |
| Linear Algebra - Foundations to Frontiers | Covered by Math 2660 @ Auburn | |
| Introduction to Linux | | | 
| Descriptive Statistics | | |

#### 3rd Year

| Class | Completed | Repository |
| ----- | --------- | ---------- |
| Genetics 1 | | |
| Algorithmic Thinking 1 | | |
| Algorithmic Thinking 2 | | |
| Probability | | |
| Cell Biology | | |
| High Performance (?) | | |
| Differential Equations | In progress | |
| Biostatistics 1 | | |
| Biostatistics 2 | | |

#### 4th Year

| Class | Completed | Repository |
| ----- | --------- | ---------- |
| Genetics 2 | | |
| Data Science | | |
| Inferential Statistics | | |
| Molecular Biology 1 | | |
| Molecular Biology 2 | | |
| Molecular Biology 3 | | |
| Bioinformatics 1 | | |
| Bioinformatics 2 | | |
| Bioinformatics 3 | | |
| Bioinformatics 4 | | |
| Bioinformatics 5 | | |
| Bioinformatics 6 | | |
| Bioinformatics 7 | | |
| Evolution | | |

#### Extra Year 

| Class | Completed | Repository |
| ----- | --------- | ---------- |
| Introduction to Machine Learning | | |
| Deep Learning | | |
| Genomic Data Science Specialization | | |

### Six Sigma and Lean

| Class | Completed | Repository |
| ----- | --------- | ---------- |
| Six Sigma: Define and Measure | | |
| Six Sigma: Analyse, Improve, Control | | |
| Lean Production | | |

### Music

| Class | Completed | Repository |
| ----- | --------- | ---------- |
| Fundamentals of Audio and Music Engineering: Part 1 | In Progress | n/a |

### Misc

| Class | Completed | Repository |
| ----- | --------- | ---------- |
| Introduction to Steel | 8/14/2017 | n/a |


### Future

* [Transfer Functions](https://www.edx.org/course/transfer-functions-laplace-transform-mitx-18-03lx)

* [Dynamics and Control](https://www.edx.org/course/dynamics-control-upvalenciax-dc201x-2#!)
