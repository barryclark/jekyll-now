...
void test(char *test) {
    ...
    if(test[0] == '1') {
        unsigned char *key = new unsigned char[8]{0x06, 0x1b, 0xda, 0x66, 0xb6, 0x74, 0x7e, 0x15};
        unsigned char *iv = new unsigned char[8]{0x2B, 0x7E, 0x15, 0x16, 0x28, 0xAE, 0xD2, 0xA6};
        unsigned char *plaintext = new unsigned char[16]{0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF};
        des_encryption(plaintext, key, ciphertext, iv, 16);
    } else if(test[0] == '2') {
        unsigned char *key = new unsigned char[8]{0x36, 0xf1, 0x83, 0x57, 0xbe, 0x4d, 0xbd, 0x77};
        unsigned char *iv = new unsigned char[8]{0x69, 0xdd, 0xa8, 0x45, 0x5c, 0x7d, 0xd4, 0x25};
        ...
        unsigned char *key = new unsigned char[8]{0x37, 0xf1, 0x83, 0x57, 0xbe, 0x4d, 0xbd, 0x77};
        unsigned char *iv = new unsigned char[8]{0x69, 0xdd, 0xa8, 0x45, 0x5c, 0x7d, 0xd4, 0x25};
        unsigned char *plaintext = new unsigned char[16]{0x32, 0x43, 0xF6, 0xA8, 0x88, 0x5A, 0x30, 0x8D, 0x31, 0x31, 0x98, 0xA2, 0xE0, 0x37, 0x07, 0x34};
        des_encryption(plaintext, key, ciphertext, iv, 16);
    } else if(test[0] == '3') {
        cout << "Change one bit of the plaintest - repeat of -test 2, except first byte of key is +1" << endl;
        unsigned char *key = new unsigned char[8]{0x36, 0xf1, 0x83, 0x57, 0xbe, 0x4d, 0xbd, 0x77};
        unsigned char *iv = new unsigned char[8]{0x69, 0xdd, 0xa8, 0x45, 0x5c, 0x7d, 0xd4, 0x25};
