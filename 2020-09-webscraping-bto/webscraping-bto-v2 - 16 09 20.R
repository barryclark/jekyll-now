...
get_bto_app_ratios_3Rplus <- function(period){
  ...
  #this is used further down to handle the scraping and cleaning differently
  #as table formats were different prior to Nov 15
  

  if(period != "Aug20"){         #used to distinguish the URL differences
...
write_csv(combined_data, "scraped_data-v2.csv") # not necessary, of course
...
## cross check scraping by comparing scraped TOTAL versus constructed total ##
...
ggsave("45r_applicants.png", dpi = 200)
...
ggsave("first_timer_ratio.png", dpi = 200)
...
ggsave("first_timer_ratio_facet.png", dpi = 200)
