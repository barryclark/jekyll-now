...
- clubSandwich: [![](http://cranlogs.r-pkg.org/badges/last-month/clubSandwich)](https://CRAN.R-project.org/package=clubSandwich)
- ARPobservation: [![](http://cranlogs.r-pkg.org/badges/last-month/ARPobservation)](https://CRAN.R-project.org/package=ARPobservation)
- scdhlm: [![](http://cranlogs.r-pkg.org/badges/last-month/scdhlm)](https://CRAN.R-project.org/package=scdhlm)
- SingleCaseES: [![](http://cranlogs.r-pkg.org/badges/last-month/SingleCaseES)](https://CRAN.R-project.org/package=SingleCaseES)
...
Pusto_pkg_dat <- 
  downloads_past_six %>%
  filter(package %in% Pusto_pkgs)

meta_pkg_dat <- 
  downloads_past_six %>%
  filter(package %in% meta_pkgs)

title_str <- paste("Average monthly downloads of R packages from", as_date(as_date(to_date) - duration(6, "months")),"through",to_date)

ggplot(downloads_past_six, aes(x = package, y = count)) +
  geom_col() + 
  geom_col(data = Pusto_pkg_dat, color = "green", size = 1.5) + 
  geom_col(data = meta_pkg_dat, color = "purple", size = 1.5) + 
  scale_x_discrete(labels = NULL) + 
  scale_y_log10(breaks = c(20, 50, 200, 500, 2000, 5000, 20000, 50000, 200000), labels = scales::comma) + 
  labs(x = "Packages", y = "Downloads (per month)", title = title_str)
