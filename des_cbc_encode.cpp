...
void test(char *test) {
    ...
    if(test[0] == '1') {
        unsigned char *key = {0x14, 0x0b, 0xb2, 0x2a, 0xb4, 0x06, 0xb6, 0x74};
        unsigned char *iv = {0x4c, 0xa0, 0x0f, 0xd6, 0xdb, 0xf1, 0xfb, 0x28};
        unsigned char *plaintext = {0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF}; //16
        des_encryption(plaintext, key, ciphertext, iv, 16);
    } else if(test[0] == '2') {
        unsigned char *key = {0x32, 0xF6, 0xA8, 0x31, 0x98, 0xA2, 0xE0, 0x37};
        unsigned char *iv = {0x0f, 0xf4, 0xc8, 0xd6, 0x1e, 0x80, 0x06, 0x18};
