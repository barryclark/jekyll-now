...
plot_projects <- function(x, filename = "project_plot.jpg",
  ...
  rect(xleft = rep(par("usr")[1], nrow(x)),
       xright = x$prop_time,
...
plot_clients <- function(x, filename = "client_plot.jpg",
  ...
  mtext(text = "Proportion of time spent\nper client", 1,
        at = xmax / 2, line = 4.20, cex = 1.75)
  rect(xleft = rep(par("usr")[1], dplyr::n_distinct(x$client_name)),
       xright = x$prop_time,
...
plot_category <- function(x, filename = "category_plot.jpg",
  ...
  x <- x %>%
    dplyr::group_by(client_category) %>%
    dplyr::summarise(prop_time = sum(prop_time)) %>%
    dplyr::arrange(dplyr::desc(prop_time))
  ...
  mtext(text = x$client_category, side = 2, las = 1,
        at = rev(seq(1, nrow(x))), line = .9, cex = 1.2)
  mtext(text = "Proportion of time spent\nper category", 1,
        at = xmax / 2, line = 4.20, cex = 1.75)
  rect(xleft = rep(par("usr")[1], dplyr::n_distinct(x$client_category)),
       xright = x$prop_time,
       ybottom = rev(seq(1,nrow(x))) -0.25,
       ytop = rev(seq(1,nrow(x))) +0.25,
       col = cols, lwd = 1.4)
