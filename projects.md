

## 2016-2017 Projects (Source Code Links)

  ✔ **TipCalc** - Basic tip calculator built with Javascript

<p data-height="265" data-theme-id="0" data-slug-hash="OXZwOR" data-default-tab="js,result" data-user="emilypmendez" data-embed-version="2" data-pen-title="Basic Calculator for Tips" class="codepen">See the Pen <a href="http://codepen.io/emilypmendez/pen/OXZwOR/">Basic Calculator for Tips</a> by Emily Mendez (<a href="http://codepen.io/emilypmendez">@emilypmendez</a>) on <a href="http://codepen.io">CodePen</a>.</p>
<script async src="https://production-assets.codepen.io/assets/embed/ei.js"></script>

  [**View Source Code**](https://github.com/emilypmendez/TipCalc)
    
    
    
    
  ✔ **BalloonTap** - Mobile app game via the Lua language built on Corona SDK

  [**View Source Code**](https://github.com/emilypmendez/BalloonTap)
  
  
  
    
  ✔ **MemoryKeeper** - Mobile app built for reminders with HTML, CSS, and Javascript
  
  [**View Source Code**](https://github.com/emilypmendez/MemoryKeeper)
    
    
  
  
  ✔ **PriorityApp** - Time management app built with Javascript and PHP
  
  [Prototype App](https://projects.invisionapp.com/static-signed/live-embed/127470428/183406726/1/latest/TNwSXy8641hvCnQUIoNiRd9NqTADfZj1LRqlEJaHlESZzlEkzrX9TBs7rRElEuXw41wyA6C2wYPklELtnGlEPf354KwlE/the-fun-life-app-project-HOME.png)
  
  [**View Source Code**](https://github.com/emilypmendez/PriorityApp)
  
  
  
  ✔ **Website Login System** - Built with PHP
  
  [**View Source Code**](https://github.com/emilypmendez/php-login)
  
  
  ✔ **Animated Slideshow** - Built with HTML, CSS, and Javascript
  
  <p data-height="265" data-theme-id="0" data-slug-hash="oLAybp" data-default-tab="js,result" data-user="emilypmendez" data-embed-version="2" data-pen-title="Making & Breaking Slideshows" class="codepen">See the Pen <a href="http://codepen.io/emilypmendez/pen/oLAybp/">Making & Breaking Slideshows</a> by Emily Mendez (<a href="http://codepen.io/emilypmendez">@emilypmendez</a>) on <a href="http://codepen.io">CodePen</a>.</p>
<script async src="https://production-assets.codepen.io/assets/embed/ei.js"></script>




## Contact me

# [Email: emilypmendez@gmail.com](mailto:emilypmendez@gmail.com)

